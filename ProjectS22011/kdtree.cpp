					...
					struct feature*** nbrs, int max_nn_chks )
{
	...
	while( min_pq->n > 0  &&  t < max_nn_chks )
	{
		...
		for( i = 0; i < expl->n; i++ )
		{
			...
			bbf_data1 = (bbf_data *) malloc( sizeof(bbf_data ) );
			if( ! bbf_data1 )
			{
						   ...
						   CvRect rect, int model )
{
	...
	sp_nbrs = (feature** )calloc( k, sizeof( feature ) );
	for( i = 0; i < n; i++ )
	{
		if( model )
...
static struct kd_node* kd_node_init( struct feature* features, int n )
{
	...
	kd_node1 = (kd_node *)malloc( sizeof( kd_node ) );
	memset( kd_node1, 0, sizeof( kd_node ) );
	kd_node1->ki = -1;
