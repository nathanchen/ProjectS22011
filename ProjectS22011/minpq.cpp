...
struct min_pq* minpq_init()
{
	...
	min_pq1 = (min_pq*)malloc( sizeof( min_pq ) );
	min_pq1->pq_array = (pq_node *)calloc( MINPQ_INIT_NALLOCD, sizeof( pq_node ) );
...
int minpq_insert( struct min_pq* min_pq, void* data, int key )
{
	...
	if( min_pq->nallocd == n )
	{
		//min_pq->nallocd = array_double( &min_pq->pq_array, min_pq->nallocd, sizeof(pq_node) );
		if( ! min_pq->nallocd )
		{
